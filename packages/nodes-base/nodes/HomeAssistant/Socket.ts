...
export function createSocket(auth: Auth, hassUrl: string): Promise<HaWebSocket> {
	...
	function connect(
		...
		const socket: WebSocket = new WebSocket(hassUrl);
	...
	return new Promise((resolve, reject) => connect(1, resolve, reject));
}